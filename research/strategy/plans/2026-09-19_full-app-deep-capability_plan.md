...
#   the providers array of polint check --format json is byte-identical before and after (no row for abstract_domains in
#   either capture, outcome.rs:726-732); a dependency_blocked or budget_exceeded outcome WOULD appear as a row, so the
#   unchanged array plus the absent stage row proves not-selected
