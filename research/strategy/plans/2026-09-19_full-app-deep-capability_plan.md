...
POLINT_FACT_ROWS_REPO=<scratch checkout> POLINT_FACT_ROWS_PATHS=<885-file scope> POLINT_FACT_ROWS_CAP=calls \
  ...
  cargo test -p polint --lib --all-features --locked --release eval::fact_rows_dump::dump_fact_rows -- --exact --ignored --nocapture
diff -r "$POLINT_GATE_OUT/rows-before-calls" "$POLINT_GATE_OUT/rows-after-calls"
#   expected: domain_observations and domain_events files absent after; summary_control differs only in
#   payload digests of rows whose before line carries exit:DoesNotReturn (count reported); every other family identical
...
**Interfaces.** Above. The `ControlFlow` SDK view's behaviour is unchanged (I2); its implementation reads the dominator trees through the `DomTree` queries, vacuous rule included.
...
| G1c | W3, W6 (I1b); W5 obligation 1 | the `eval::fact_rows_dump::dump_fact_rows` ignored test entry run from the branch-base worktree and from the checkout under test on the G1 cells; `diff -r` per family; `diff` of stdout | every family byte-identical except those the workstream lists (W3 on `calls`: `domain_observations`, `domain_events` absent, `summary_control` key set identical with a counted payload-digest delta on rows whose before line carries `exit:DoesNotReturn`; W6: none); diagnostics digest and stdout identical | all fact rows |
