...
| tree-RSS sampler | research doc section 7 (revised) | `.scale-envelope/rssrun.py:22-58` (tree walk via `/proc/<pid>/task/<pid>/children`, `VmRSS` sum), `:75-107` (JSON summary with `peak_rss_bytes`, `wall_s`, `RLIMIT_AS` guard); the timeline is one tree total per sample (`:80-90`, `:114`) until W9 commit 1 adds per-process records (`rssrun-timeline-2`) | replaces `/usr/bin/time -v` for memory; per-process samples are new work (round 6) |
...
- **I5, determinism.** `cargo test -p polint --lib eval::determinism_gate --locked` (the N=10 seeded-permutation gate, `.github/workflows/ci.yml:247-248`) stays green in every commit; W6 extends its fixture set. From W6 on, determinism across thread counts (G7's one-versus-twelve-thread diff) is decidable only because every budget the per-unit stages consume is local to a function, a unit or an SCC (W3's per-function solver cap, W4's per-function dominance estimate against a run-wide ceiling value that is an input, W8's per-SCC closure cap); a counter shared across units and consumed in schedule order would make the set of `BudgetExceeded` rows follow the rayon schedule, and section 2.1 enumerates the interactions that were checked for that (round 6).
...
|---|---|---|
| W3 solver budget × W6 parallel per-unit domains × W7 shard | a per-run iteration total consumed across units | would make a unit's domain rows depend on the other units in the run and on the rayon schedule, outside every shard-key field; resolved by making the per-function cap the only budget input (W3), with the run total reported as a counter; forced by W3 test (i), W6 test (h) and the W7 run-scope test |
...
| W4 dominance ceiling × W7 shard | `POLINT_CFG_MAX_DOMINANCE_PAIRS` value; the per-function estimate `worst_case_dominance_pairs` is summed over the run (`cfg/budget.rs:66-79`) | the ceiling value is in `parameter_digest`; the trip decision today is run-wide (the sum over every function in the run against one ceiling), so under W6 the decision is taken once per run before any unit is lowered from the run-wide estimate and handed to every unit as an input, never recomputed per unit; the resolved `DominanceMaterialization` therefore joins `parameter_digest` beside the ceiling value, and W7's forcing test for the ceiling dimension changes the resolved mode, not only the env value |
...
| W9 memory ceiling mid-provider × W7 shard writes | the ceiling can trip between units | a shard is written only for a unit whose lowering completed; skipped units write nothing and are recorded as `budget_exceeded`; shards written before the trip are valid because their bytes depend on no tripped state |
...
// domains/solver.rs
pub struct SolverBudget { pub max_iterations_per_function: u32, pub widening_fuel: u32 }   // round 6: no per-run total
...
The provider output digest is the digest over `(UnitId, unit.input_digest, unit_graph.digest)` in `UnitId` order, followed by the sorted (canonical stable-key text, payload digest) rows of `call_targets` and `unresolved_calls`, which the provider emits from the whole-program post-merge pass and which no unit graph contains (round 6: without them the digest was a valid cache key but not a fingerprint of every family the provider produces, and I1a could not see a regression in those two families when W8 moves the pass onto `CrossUnitIndex`); per-unit digests are what W7 persists.
...
- `.scale-envelope/rssrun.py`, extended in this commit (round 6; the only `.scale-envelope` script this plan changes). Today each timeline sample is one `(t, tree_total)` pair: the loop sums `VmRSS` over `descendants(proc.pid)` into `total` and appends `(now, total)` (`rssrun.py:80-90`), and the timeline is written as that list (`:112-114`). No per-pid figure and no record of which pids were alive survive, so G2b's per-process question is not answerable from the committed tool. The commit changes the sample to a per-process record and labels the file `rssrun-timeline-2`: `{"summary": {..., "timeline_schema": "rssrun-timeline-2", "root_pid": <pid rssrun.py spawned>}, "timeline": [{"t": 0.2, "total": <bytes>, "procs": [[<pid>, "<comm>", "<argv0 basename>", <rss bytes>], ...]}, ...]}`, where `comm` is `/proc/<pid>/comm` and `argv0` is the first NUL-separated field of `/proc/<pid>/cmdline`, both read once per pid and cached because they are constant for a pid's lifetime; `total` is the same sum as today, and `summary` keeps `label`, `cmd`, `exit_code`, `wall_s`, `peak_rss_bytes`, `peak_rss_gb` and `samples` unchanged (`:102-110`), so the `"peak_rss_gb"` grep in section 5.1 and every existing reader of the summary line are unaffected. The sampler's interval stays 200 ms (`:20`).
- `overlap.py`: `scripts/deep-gate/overlap.py <timeline.json> [--sidecar polint-go-frontend]` reads `rssrun-timeline-2` and classifies each sample's `procs` by `argv0` basename: the polint process is `root_pid`; a Go semantic sidecar process is any pid whose `argv0` basename equals the frontend binary name `polint-go-frontend` (`go/semantic/process.rs:101-106`, `frontend_binary_name`; `.exe` on Windows); everything else (`polint-go-symbols`, a `go` toolchain process building a sidecar, a rule host under `polint check`) is `other` and is listed with its basename. It prints six numbers per run: the sidecar-present window (first and last sample time at which at least one sidecar process exists, or `none`), the maximum polint-process RSS inside that window, the maximum polint-process RSS outside it, the maximum tree `total`, and the sample count. Those are the quantities G2b and G6 bind or report. The sidecar is never identified by assuming a single child or by its position in the tree; a timeline with the old schema (no `timeline_schema` field) is rejected with a message naming this commit.
...
| `timeline.json` per-sample `procs` (`pid`, `comm`, `argv0`, `rss`), `timeline_schema`, `root_pid` | W9 commit 1 (`rssrun-timeline-2`) | new work |
| `overlap.py` window and maxima | W9 commit 1 (`scripts/deep-gate/overlap.py`) | new work |
...
| `digests.py` `N/N`, `DIFFER`, `MISSING` | `.scale-envelope/digests.py:5-24` over the `stage done` rows | exists |
...
| G2b | W5, W6; prerequisite W9 commit 1 | `probe s885 calls <885-file scope>` (the helper always passes `--timeline`); `python3 scripts/deep-gate/overlap.py "$POLINT_GATE_OUT/s885.timeline.json"` over the per-process timeline W9 commit 1 adds (`rssrun-timeline-2`; today's sampler records one tree total per sample, `rssrun.py:80-90`, `:114`, and cannot answer this gate, round 6) | `overlap.py`'s "max polint RSS inside the sidecar window" under 5,265 MB, i.e. polint-process RSS under that figure at every 200 ms sample at which a `polint-go-frontend` process is present (today about 6,843 MB by subtraction, 13,689 tree minus 6,846 sidecar, a peak-minus-peak estimate the per-sample reading replaces); this is the necessary condition for G6's 12 GB tree ceiling, stated in the research doc section 7, measured here before the full backend is attempted | the tree peak on this cell is reported, not bound |
...
| G9 | W7, W8 | cold probe; edit one Go file in a leaf unit; `KEEP_CACHE=1 probe g9-warm calls <core>`; `stages.py` on the warm stderr | three lines, the first bound and the other two reported (round 6): (1) Rust-side warm pipeline under 30 s, defined as the sum of every `stage done` `elapsed_ms` except `polint.go.semantic`'s; (2) the sidecar floor: `polint.go.semantic`'s `elapsed_ms` and the sidecar's own `go_semantic.phase.*.elapsed_ms` counters, expected 25 to 27 s of sidecar work plus the row lowering, because the sidecar cache key folds the whole-scan `go.syntax` output digest (`go/semantic/cache_key.rs:32-45`; `upstream_digest` is that digest, `go/semantic/provider.rs:440`; the prefetch starts from the kernel's `upstream_digests` after `go.syntax`, `analysis_kernel/mod.rs:402-408`), so one edited file misses it and re-runs whole-program `packages.Load` plus SSA; (3) total warm wall, expected about the sidecar floor plus whatever of line (1) the prefetch does not hide; `units.touched` equals the edited unit plus dependents; stdout identical to a cold run on the edited tree. A per-package sidecar key is not in any workstream (section 7) and is recorded as a follow-up decision; until it exists the 30 s figure binds the Rust side only | first-run rows |
