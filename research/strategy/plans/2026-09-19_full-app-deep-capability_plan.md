...
# fact-row oracle: the same ignored test entry run from the branch-base worktree (before) and this checkout (after)
...
#   before parts column contains exit:DoesNotReturn and whose after parts column lacks it and is otherwise equal
#   (count reported; the parts column is the plaintext SummaryFact::payload_digest, see W9); every other family identical
...
polint unknowns --cap control_flow <885-file scope> | grep -c DomainBudget      # the per-function cuts after commit 3, through the new collector row
```
...
python3 .scale-envelope/digests.py "$POLINT_GATE_OUT/exc-before-full-relation.stderr" "$POLINT_GATE_OUT/exc-full-relation.stderr"   # 23/23; the before capture is in slot 1
probe s45-w4 calls <45-file scope>; python3 .scale-envelope/digests.py "$POLINT_GATE_OUT/s45-before.stderr" "$POLINT_GATE_OUT/s45-w4.stderr"   # 21/21; default bound == full relation at this size
```
...
| G1c | W3, W6 (I1b); W5 obligation 1 | the `eval::fact_rows_dump::tests::dump_fact_rows` ignored test entry run from the branch-base worktree and from the checkout under test on the G1 cells; `diff -r` per family; `diff` of stdout | every dumped family byte-identical except those the workstream lists (W3 on `calls`: `domain_observations`, `domain_events` absent, `summary_control` key set identical, and each differing line's plaintext parts column shows exactly the removal of `exit:DoesNotReturn`, count reported; W6: none); diagnostics digest and stdout identical | all dumped fact rows; aggregated and row-less families through I1a |
