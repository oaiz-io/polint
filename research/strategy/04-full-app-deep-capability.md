  ...
  -> polint.refined_calls        inputs include summary_call, summary_events     (provider.rs:1813-1843)
       -> polint.direct_summaries  inputs include domain_observations, domain_events (provider.rs:1568-1598)
            -> polint.abstract_domains  inputs include cfg_* and mir_*               (provider.rs:1545-1567)
                 -> polint.cfg            inputs mir_bodies, mir_blocks, ...           (provider.rs:1448-1474)
...
1. **Identity-preserving until the pivot, byte-identical after it.** Every workstream before W5 must reproduce all 23 provider output digests (the oracle X5 used). W5 changes the internal identity but must reproduce the canonical key text and every digest at the boundary; `NEXT-LEVER.md` lists the proof obligations and they are adopted unchanged.
...
probe() { # $1 tag, $2 cap, $3 paths...
  tag=$1; cap=$2; shift 2
  rm -rf .polint/cache/analysis .polint/cache/layers
  /usr/bin/time -v -o /tmp/$tag.time timeout 300 \
    polint unknowns --cap "$cap" "$@" > /tmp/$tag.stdout 2> /tmp/$tag.stderr
  echo "exit=$?"; grep -E "Maximum resident|Elapsed" /tmp/$tag.time
  grep "stage done" /tmp/$tag.stderr | sed 's/.*provider=//' | awk '{print $1, $2, $3, $6, $7}'
}
```
...
|---|---|---|---|
| 45 Go files | G1 oracle | G7 | |
| 885 Go files | G1 oracle, G2, G1b | G7 | G9 |
| 1,588 Go files | G1b, curve point | | |
| 4,752 Go files (full backend) | G3, G4, G5, G6, G10 | G6 warm | G9 |
| 2,381 TS files (full frontend) | G8 | | |
| excalidraw (public, 385 TS files) | G1 oracle against `.scale-envelope` X6 | | |

Digest comparison uses `.scale-envelope/digests.py`, which extracts the `provider=... digest=...` pairs from two stderr captures and prints the count of identical providers with any `DIFFER` or `MISSING` rows; it is the same tool that caught the X6 false positive.
...
| G1 identity oracle after each targeted fix | W1, W2, W4 | `.scale-envelope/digests.py` comparison of per-provider `digest=` values in `stage done` rows, before and after, on excalidraw and on the 885-file consumer scope | 23 of 23 digests identical; `polint check` diagnostics digest identical |
| G1b cost split confirmed | W1 | temporary `polint::probe` step rows inside `lower_go_mir` around `lower_file`, `finish_with_types`, `lower_control_flow` and `normalized`, as X4 did for evidence, on the 885 and 1,588-file scopes | the `lower_control_flow` and `matching_function` shares match section 3.8's ordering, or the section is corrected before W1 is designed |
| G2 885-file scope | W1, W2 | `probe s885 calls <885-file scope>` | wall under 60 s; peak under 8 GB (today 152.3 s, 13.7 GB) |
| G3 `semantic_mir` on the full backend | W1, W2, W6 | `probe full-mir calls <core>` and read the `polint.semantic_mir` stage row | stage under 60 s; `rss_delta_mb` under 3,000; `key_mb` growth under 1,500 (today 565 s, +10,415 MB, 3,448 MB) |
| G4 `cfg` on the full backend | W4 | same run, `polint.cfg` stage row and step rows | stage under 30 s; `dominators` step under 5 s |
| G5 domains no longer on the `calls` path | W3 | same run; `polint.abstract_domains` absent from the stage rows when only `calls` is requested, present when `control_flow` domains are requested | present or absent exactly as the family demand says; `polint unknowns` reports the budget row when it does run |
| G6 full backend `calls` completes | W1 to W6 | `probe full-calls calls <core>` | exit 0; wall under 300 s; tree peak under 12 GB; all 21 selected providers have a stage row |
...
| G8 full frontend `calls` completes | W6 (TS lowerer twin fixes) | `probe full-ts calls <frontend paths>` | exit 0; wall under 300 s; peak under 12 GB (today `semantic_mir` alone is 203 s) |
...
- Build: `lower_file` and `push_body` take pre-built maps (function by `(file, name, span)`, package by file, module node by file); `go_closure_capture_names` and its TS twin use the indexed `references_for_file` and a `definitions_by_symbol` index, exposed through `AnalysisHost` overrides in `impl AnalysisHost for AnalysisDb` (`core/db.rs:6043`); `lower_control_flow` groups operations and effects by body once (`go/mir/lower.rs:115-124`, `ts/mir/lower.rs:132-141`); `Icfg::from_facts` indexes CFG nodes by operation (`ifds/mod.rs:93-98`); the refined-calls Go semantic join indexes native call sites by `(file, span)` and functions by `(file, name)` (`refined_calls/provider.rs:361-454`); `owner_symbol` indexes symbols by `(file, name, span)` (`calls/extract.rs:554-567`). Delete the duplicate `normalized` call on the lowerer's return (`go/mir/lower.rs:99`, `ts` twin) since `SemanticStore::from_output` normalises again (`store.rs:38`).
...
- Build: replace `dominator_relation_with_extra_exit` (`cfg/derived.rs:339-400`) with the Cooper-Harvey-Kennedy iteration over an `idom: Vec<Option<BlockIndex>>` in reverse postorder (the CFG already records `reverse_postorder` per block, `cfg/facts.rs` `BasicBlockFact`), for both directions; emit tree edges always and the closure only when the budget allows, computing the closure from the tree rather than the other way round. Control dependence follows from the post-dominator tree (`derived.rs:203-257`).
...
| W3 | presence of `polint.abstract_domains` and `polint.direct_summaries` rows on a `calls`-only request; `facts` total | domains absent on `calls`; total facts fall by the domain family size | `polint.refined_calls` `digest=` (Q4) |
| W4 | `polint.cfg` step rows `dominators`, `postdominators`, `control_dependence` | near-linear; `elapsed_ms` drops | `digest=` when `POLINT_CFG_MAX_DOMINANCE_PAIRS=0`; the set of tree edges otherwise |
| W5 | `key_mb` on every row | order-of-magnitude drop; `keys` unchanged | every `digest=`; `facts` |
| W6 | `polint.semantic_mir` and `polint.cfg` `elapsed_ms` and `rss_delta_mb`; busy cores | parallel speed-up; retained delta per unit, not per program | ai-friendly stdout bytes across permutations |
...
| 3.1 | `internal_core/stable_key.rs:62-73`, `:78-88`, `:106-112` | `intern`, `intern_and_resolve` (write lock), `resolve` (Arc clone) |
...
| gating | `analysis_kernel/provider.rs:1013-1028`, `:1070-1090`, `:1094-1130`; `analysis_kernel/mod.rs:549-566` | seeds, closure, cross-file gate |
