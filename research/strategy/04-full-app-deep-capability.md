...
probe() { # $1 tag, $2 cap, $3... paths; POLINT_GATE_TIMEOUT (s, default 300); KEEP_CACHE=1 for warm cells
  ...
  POLINT_CACHE_DIR="$POLINT_GATE_CACHE" python3 .scale-envelope/rssrun.py --label "$tag" --as-limit-gb 28 \
...
| G1c fact-row oracle | W3, W6 (and W5 obligation 1) | per-family sorted (canonical key text, payload digest) dump from the ignored test entry, run from the branch-base worktree and from the checkout under test, on the same cells as G1 | byte-identical for every family except those the workstream names as changed (W3 on `calls`: `domain_*` absent, and `summary_control` with an identical key set and a counted payload-digest delta confined to rows whose before digest carried `exit:DoesNotReturn`; W6: none); diagnostics digest and ai-friendly stdout identical |
...
| W3 | presence of the `polint.abstract_domains` row on a `calls`-only request; `facts` total; the `digest=` of `direct_summaries`, `type_value_alias`, `semantic_graph`, `solver`, `refined_calls` | domains absent on `calls`; total facts fall by the domain family size; those five digests move because their recipes fold the now-absent `abstract_domains=` digest | every other `digest=`; G1c fact rows of every family except `domain_*` and the counted `summary_control` payload delta; diagnostics digest; stdout (Q4) |
