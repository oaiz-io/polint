...
| [04-full-app-deep-capability.md](04-full-app-deep-capability.md) | Why does the deep graph capability (semantic MIR, CFG, dataflow, RTA) fail at full-application scale, and what architecture fixes it? | Added 2026-09-19, after the full-application benchmark of 2026-09-18. Root-causes the measured wall (a 565 s, +10.4 GB `semantic_mir` on a 4,752-file Go backend; a kill at 634 s and 18.3 GB inside `cfg`) to five defects in the Rust graph layer: composed-text identity retained for the run, quadratic joins in the lowerers and their consumers, per-fact digest materialisation, a whole-program domain solver that exhausts its budget and then fans out, and naive dominator computation. Surveys how CodeQL, Glean, Kythe, SCIP, Joern, Souffle, Infer, Salsa, rustc, gopls and the IFDS literature avoid each. Compares five architectures and recommends scoped-deep analysis units on a dense-identity core with persisted unit shards, sequenced as nine dependency-ordered workstreams with probe-verified acceptance gates. Eight of its nine questions were then resolved by research inside the document (persistence scope, trait privacy, shard layout, domains off the `calls` path, budget semantics, the TS unit, naming, threshold host); one owner decision remains, how the acceptance-gate runner is provisioned. Referred to by its slug, not as "report 04", which stays the evaluation report. |
