...
module.exports = grammar({
  ...
  rules: {
    ...
    // Go 1.26 allows `new(expr)`. Prefer `_type` when both a type and an
    // expression match (bare `new(x)`, `new(pkg.T)`) so existing programs keep
    // the tree-sitter-go 0.25.0 shape: identifiers stay `type_identifier`.
    // `prec.dynamic(2)` offsets `_type_identifier`'s dynamic -1 so types win
    // the `[_simple_type, _expression]` conflict; dynamic 1 ties and expressions win.
    special_argument_list: $ => seq(
      '(',
      optional(seq(
        choice(prec.dynamic(2, $._type), $._expression),
